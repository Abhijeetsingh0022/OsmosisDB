...
def extract_sql(msg: PGMessage) -> str | None:
    ...
    if msg.msg_type == MsgType.PARSE:
        ...
        # We need the query string which is between the first and second \x00.
        try:
...
def is_startup_message(data: bytes) -> bool:
    ...
    # Startup: length(4) + protocol_version(4) 196608 = 3.0
    # SSL request: length(4) + 80877103
    # Cancel request: length(4) + 80877102
    if length < 8:
        return False
